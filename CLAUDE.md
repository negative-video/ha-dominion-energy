...
- **`green_button.py`**: Pure ESPI/Green Button parsing and timestamp calibration, **no Home Assistant imports**. Dominion's exports carry a constant timestamp offset that is *not derivable from the file* — a real August export measured **+5h** against the API, a February one **+4h**. `best_alignment()` measures it by correlation against known-good data; `apply_shift()` applies it uniformly.
  **Two traps, both already paid for:**
  1. **Do not model the offset.** An earlier version reconstructed the intended wall clock and re-localized with DST rules. It made two exports agree with each other at 100% while leaving both five hours from the truth. Export-vs-export agreement proves nothing; only comparison against independently-correct data does.
...
- **`usage.py`**: Pure helpers with **no Home Assistant imports** — interval filtering, hourly aggregation, UTC de-duplication, cumulative-sum building, billing-period boundary math, and `find_cost_anomalies()` (see *Catching a bad cost day*). This is where testable logic belongs: `coordinator.py` cannot be imported without Home Assistant, so anything decidable should live here (or as a module-level helper in `coordinator.py`) rather than inside a method.

  **The billing period's end is an estimate, and has to be.** The API publishes no next-meter-read date — `dompower` 0.4 probed 46 leaf keys across actionCodes 1, 3, 4 and 5 and found none — so `billing_period_end()` returns `current_period_start` plus `DEFAULT_BILLING_PERIOD_DAYS`. The field that looks like an end, `usage_through_date` (`current_period_end` before 0.4), is the day `current_usage_kwh` is measured *through*: it advances daily and is never later than today, which is why `period_end_is_trustworthy()` rejects it. Do not carry the previous cycle's length forward instead — a long cycle is followed by a short one that pulls the read date back (33 days in January 2026, then 28 in February), so that propagates the swing the wrong way.
...
`_async_update_data()` therefore catches `UpdateFailed` and falls back to the last good payload for `STALE_DATA_GRACE` (24 hours) instead of letting a refused poll take every entity unavailable. Four things about that are load-bearing:
...
2. **Only `UpdateFailed` is absorbed** — `tests/test_features.py` asserts it twice, once on the handler list and once by driving a `ConfigEntryAuthFailed` through a real cycle. It must pass straight through, or the "Reauthenticate" button never appears and the integration sits frozen on day-old numbers instead. It is also not counted as a degraded cycle: the failure counter is for an API that has stopped answering, not for credentials that need attention.
3. **Serving a stale payload must not re-stamp it.** `_serve_last_good_data()` returns the cached object untouched; only the success path calls `replace(data, last_success=dt_util.now())`. Stamping the served copy is the natural-looking mistake, and it would turn a 24-hour window into an unbounded one — each failed cycle handing back data marked fresh as of that failure, so the window measures the outage instead of the data's age and never closes.
4. **`last_successful_update` is the entity that tells the truth.** Since the other sensors keep reading through a failure, "the sensors have values" no longer proves the API is answering; this one stops advancing the moment cycles start failing. Diagnostics reports `consecutive_failures` alongside it.
...
Both paths land on the same code, and the tag is what guarantees it: the release workflow builds the wheel from the tagged commit, and `uv.lock` pins the commit the tag resolves to. Verified for v0.3.0 and again for v0.4.0 — all nine files in the release wheel are byte-identical to the git-installed copies.
